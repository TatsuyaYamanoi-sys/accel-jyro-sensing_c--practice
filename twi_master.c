...
void tw_init(twi_freq_mode_t twi_freq_mode, bool pullup_en)
{
	...
	{
		case TW_FREQ_100K:
		/* ビットレートレジスタ72とプリスケーラを1に設定すると、
           SCL_freq = 16MHz /（16 + 2 * 72 * 1）= 100KHz	*/
		TWBR = 72;
		break;
		
		case TW_FREQ_250K:
		/* ビットレートレジスタ24とプリスケーラを1に設定すると、
		SCL_freq = 16MHz/(16 + 2*24*1) = 250KHz	*/
		TWBR = 24;
		break;
		
		case TW_FREQ_400K:
		/* ビットレートレジスタ12とプリスケーラを1に設定すると、
